...
@Entity
@Table(name = "SCD_DC_V_SHOP_OPCL_DAT", schema = "SCD")
@Data
@Builder
@NoArgsConstructor
@AllArgsConstructor
...
public class StoreOpenStoreCloseDate implements IEntity<Long> {
    @Id
    ...
    private LocalDate manualOpenDate;
    ...
    @JsonFormat(pattern = "dd.MM.yyyy")
    private LocalDate manualCloseDate;
    @ManyToOne
    @JoinColumn(name = "ID_DEPARTMENT", insertable = false, updatable = false)
    @View(flat = true)
    @ViewField(editable = false, required = false)
    private LocationDepartmentDates dates;
    @ManyToOne
