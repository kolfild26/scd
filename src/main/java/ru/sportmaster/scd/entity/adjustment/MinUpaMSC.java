...
@Entity
@Table(name = "SCD_DC_V_COLL_MSC_SHOP_PT_EXTS_CATEGORY_MIN_UPA", schema = "SCD")
@Data
@Builder
@NoArgsConstructor
@AllArgsConstructor
...
public class MinUpaMSC implements IEntity<Long> {
    @Id
    @Column(name = "ID_COLL_MSC_SHOP_PT_EXTS_CATEGORY_MIN_UPA")
    private Long id;
    @ManyToOne
    ...
    @View(flat = true)
    @ViewField(renderer = @ViewRenderer(srcField = "name"), required = false)
    private CategoryHierarchy category;
    @ManyToOne
    @JoinColumn(name = "ID_EXT_SIZE")
    @View(flat = true)
    @ViewField(renderer = @ViewRenderer(srcField = "name"), required = false)
    private ScaleSize extSize;
